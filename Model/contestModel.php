...
class contestModel extends DB {
	...
	public function get_problem_list($contest_id) {
		$result = parent::query ( "SELECT inner_id, problem.pro_title, contest_pro.pro_id FROM contest_pro LEFT JOIN problem ON (problem.pro_id = contest_pro.pro_id) WHERE contest_id = ?", $contest_id );
		if ($result->rowCount () != 0) {
